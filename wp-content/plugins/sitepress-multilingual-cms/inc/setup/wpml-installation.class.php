...
class WPML_Installation extends WPML_WPDB_And_SP_User {
	...
	private function prepopulate_translations( $lang ) {
		...
		$results = $this->wpdb->get_results(
			"
			SELECT post_type, ID FROM {$this->wpdb->posts} WHERE post_status IN ('draft', 'publish','schedule','future','private', 'pending')
			"
		);

		foreach ($results as $result) {
			$count = get_option('wpml_trid_count');
            $maxtrid = get_option('wpml_prefix_hash')*100000 + $count;

			$this->wpdb->query(
				"
			INSERT INTO {$this->wpdb->prefix}icl_translations(element_type, element_id, trid, language_code, source_language_code)
			VALUES ('post_{$result->post_type}', {$result->ID}, {$maxtrid}, '{$lang}', NULL)
			"
			);
		...
		foreach ( $taxonomies as $tax ) {
			...
			$select_query
							= "
				SELECT %s, term_taxonomy_id, %d AS trid, %s, NULL FROM {$this->wpdb->term_taxonomy} WHERE taxonomy = %s
				";
			$select_prepare = $this->wpdb->prepare( $select_query, array( $element_type, $maxtrid, $lang, $tax ) );
			$resultsTaxonomies = $this->wpdb->get_results( $select_prepare );

			if(!empty($resultsTaxonomies)) {
				$this->wpdb->query(
				"
				INSERT INTO {$this->wpdb->prefix}icl_translations(element_type, element_id, trid, language_code, source_language_code)
				VALUES ('{$resultsTaxonomies[0]->tax_category}', {$resultsTaxonomies[0]->term_taxonomy_id}, {$resultsTaxonomies[0]->trid}, '{$lang}', NULL)
				"
				);
				$count++;
				update_option( 'wpml_trid_count', $count, 'no' );
		...
		$resultsComments = $this->wpdb->get_results(
			"
			SELECT comment_ID AS ID FROM {$this->wpdb->comments}
			"
			);

		foreach ($resultsComments as $resultsComment) {
			$count = get_option('wpml_trid_count');
			$maxtrid = get_option('wpml_prefix_hash')*100000 + $count;
			...
			$this->wpdb->query(
				"
				INSERT INTO {$this->wpdb->prefix}icl_translations(element_type, element_id, trid, language_code, source_language_code)
				VALUES ('comment', {$resultsComment->ID}, {$maxtrid}, '{$lang}', NULL)
				"
			);
			$count++;
			update_option( 'wpml_trid_count', $count, 'no' );
